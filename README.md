...
On **Grocery**, check items off while shopping, type extras by hand, or tap **Copy list** for a plain-text list. On browsers that block clipboard writes (notably Brave mobile), a sheet opens so you can select/long-press copy or use the system share sheet (text only, no link). The list is stored locally in the browser (`localStorage`).
